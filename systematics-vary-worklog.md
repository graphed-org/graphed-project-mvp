...
## 2026-09-09 — owner decisions on the four open items; m57 weight dedupe (plan graphed-workdir/weight-dedupe-plan.md)
Owner: (1) both idioms correct, dedupe route ("relative-delta/shift-style weights and re-using absolute weights as beginnings along different paths both work"); (2) allow numpy scalars in `_points.coordinate()`; (3) m37 emit test → lower bound ("we don't want shutdown stalls"); (4) widen mypy to tests. Items 2–4 dispatched as Agent-tool implementers (points-numpy-scalars → PR on graphed; m37-emit-lower-bound → PR on graphed-executors; mypy-widen-tests → one PR per repo, frozen trees untouched, owner table for frozen-code decisions).
- m57 gap measured on main 3b00157 (scratchpad/dedupe/probe.py): (a) two families on one central node → SF² (nominal [1,4,16] for SF=[1,2,4]); (b) ambient as nominal (tour idiom) → SF², hf_up 1.21·SF² (read through both factors), mu_up 1.05·SF²; (c) masked child, one parent factor → squared, adopted nominal node == reindexed SF; (d) masked child, two parent factors → nominal [2,8] vs physics [1,2] (sf is inside the adopted product, not a factor node). Zero frozen/extra tests register the ambient/weight(...) as a nominal; tour cells 14/34/36/38 do.
- Plan r0: the nominal names the factor by NODE identity — new / extends a registered lineage factor (members join its container, composition multiplies the member once) / extends the ambient (≥2 factors: the member IS the composition at its label); lineage matching replaces a masked child's adopted product by the parent's factors re-indexed; two-level reads preserved (m56 rule unchanged); value-equal recomputed central stays a new factor. Prototype worktree scratchpad/dedupe-impl (branch dedupe/prototype at 3b00157), Agent dedupe-prototype (Opus high) building it + measuring frozen impact; plan review workflow follows on the prototype (m56 shape: 3 lenses xhigh + 2 refuters/finding).
- Items 2–4 reviewed + ENQUEUED 2026-09-09 ~07:35 (watchers scratchpad/npscalar/, m37emit/, mypy/; monitors armed): (2) graphed#26 e1c7b07 — one shared `_tags.python_number` (numbers.Integral/Real minus bool, Fraction kept exact at the coordinate site) used by both the declaring and placement channels; 11 extra tests, 4 fail on main (3 placements + float32 precision). (3) graphed-executors#13 62de0ba — frozen m37 test_emit: SUBMITTED exact (driver-side), thread STARTED/FINISHED exact (in-process), process floor 1 (bounded deque + suppressed put_nowait + daemon flush guarantee nothing; measured 5/8 under 24/60-way load); dated "Owner decision" docstring removed on request (adjudication prose). CI runs no integrity scan (precedent #10 m49 re-freeze). (4) mypy widened to tests in four PRs — graphed#27 (+147/−74; source fix: `AwkwardBackend.op_form` param widened to `Sequence[Form]` + cast, closes 113 arg-type errors incl. the `Session(AwkwardBackend())` debt), executors#14, histogram#13 (carries `# type: ignore[arg-type]` for the op_form defect until its graphed pin 6cc2bbe advances past #27 — retire it at the next pin bump or warn_unused_ignores fails), corpus#2. Frozen trees UNTOUCHED: `tests.frozen.*` overrides disable the tripped codes per repo (graphed 22 codes / 770 errors / 218 files; executors 15 / 411 / 143; histogram 41 modules by name, no __init__; corpus `misc` on conftest ×2) — OWNER RULING PENDING on annotating frozen suites (a re-freeze). #26's new test is strict-clean under #27's config (measured). Pre-existing: executors' local suite has 29 failures from the shared venv's stale PyPI graphed-histogram 0.0.1 + pre-m54 graphed pin (CI installs from git; venv not rebuilt per standing constraint).
- m57 prototype v1 0626b82 (Agent dedupe-prototype; report scratchpad/dedupe/prototype-report.md): probe cases a–d at target; decisions D1 live factors (walk back through `_adopted` containers, not `_lineage_factors`), D2 nothing composed/re-indexed to decide (`_same_node` = equal ids + vary-link-only separation; ambient nominal off the memo — §3's literal recipe shifted node ids on masked programs, measured nosilent.py), D3 "≥2 factors" redundant, D5 extended factor inherits the adopted container's tag map, D7 nested central names nothing. D4 (collapse the ambient extension into one factor) REFUTED by absorbed.py (two prior factors → later `lf` on `sf` squares again: nominal [0.5,2,8]) → repair: ORDERED OPERATIONS (factor multiplies / overlay replaces at its labels). v2 3fb682a: D8 overlays by id in `_overlays` slot; D9 product tree kept when no overlay (byte identity vs main holds, node counts incl.); D10 `_check_forms` takes the overlay set; D11 ambient outcome is an append and folds; D12 with one factor the factor branch fires (overlays only from two factors). Measured: (i) factor after overlay multiplies, (ii) join-before-overlay keeps position (P interned, same node 8), (v) overlay through mask + ancestor extension, (vi) m56 gate/registries see overlays; memo across an intervening shift still fires (memo copied, epoch ignored by design). FROZEN IMPACT: 2 m56 tests (test_joint_oracle — fixture's JES/HF nominal 1.0 makes both centrals one node, pinned nominal = SF²; test_label_precision — probe built from a node no longer in the shared container) → disputes for the owner; 4 extra tests adapt (3 refusal tests build two factors from one expression; `_check_forms` spy arity). Tour cells 14/34/36/38: printed lines unchanged, values change ((0.5·pt)^k → 0.5·pt). Plan r1 (ordered ops, live factors, joint-of-two-families rule = cross member alone) → r2 (D8–D12 folded). Plan review workflow on v2 next.
- m57: owner affirmed correction B for the two m56 disputes (fixture nominals distinct; tests keep the two-factor product oracle). Applied on dedupe/prototype as an owner-affirmed re-freeze (precedent: m48 in m56). Items: graphed#27 merged 9374bd8; histogram#13 merged 05aed7d; executors#13/#14 in the merge queue; corpus#2 needs the owner's own merge (no merge queue there; gh pr merge blocked by the permission classifier).
- m57 plan review r2 wf_573d0563-ce7 (37 agents, 70 min; digest scratchpad/dedupe/review/r2-digest.md): STANDING RS-1 BLOCKER (overlay test read the fold memo; a join drops it → a handle read before the join is 'new' → squared; an intervening weight() read flips the answer), RS-2 BLOCKER (join discards the joining central's own shift coordinates → order-dependent ambient, jes_up 1.5 vs 2.25), FS-1 BLOCKER (`_same_node` refused project links though projections keep ids → re-derived central at graphed.nominal squares), RS-3 MED (ordered walk quadratic: 56→730 ms, 251→2141 nodes at F=64), RS-5 MED (stale prefix handle → new factor), RS-6 LOW (m56 order independence held only by the btag-only fixture), FS-5 MED (bullet 9 non-discriminating; shift-after-weight/tag-map observables unpinned). Refuted: RS-4 (association bits — pre-existing on main), DEDUPE-C1..C6, FS-2..FS-4. Nits RS-7, C7, FS-6 folded. PLAN r3: read RECORDS (nominal node + slots composed, written by the read that minted the handle; entries in slots; overlay inserted after its prefix; memo never consulted to decide), union nominal on a join (coordinate conflict refused), tree per product run, mask-only `_same_node` restriction, §4 legs for each. Prototype v3 dispatched (Agent dedupe-prototype); delta review (r2→r3) follows on v3.
- m57 prototype v3 cfef1c9 + c6794cd (extra-test adaptations): slotted operations; every `_ambient_weight` return records (nominal node, live slots) in `_reads` SHARED down vary links, reset on adoption (D13–D15); overlay position = after the prefix's last own slot, or right after an adopted head, else no match (D16); union at the container's nominal MEMBER (D17 — the top-level reading refused the m56 capstone under name identity), refusal before anything is minted (D18); order independence = labels + values, ids permute as on main (D19); overlays by slot mapped at the composition boundary (D20). Reviewer probes after: C2 both spellings ('ambient', 2) same nodes; B_stale mu_up [1.05,2.1,4.2]; A2 jes_up 2.25 both orders; moving-SF order test equal; I6 ratio 21.3× → 1.1× at F=128; proj3 [0.5,1,2]. Full runner exit 0 (66 sections), zero frozen failures, nosilent byte-identical to main. Plan r4 folds D13–D20; DELTA REVIEW (r2→r4) wf_d70cddb0-b5b launched (closure / delta-semantics / delta-tests lenses + refuters).
- m57 delta review r2→r4 wf_d70cddb0-b5b (29 agents, 41 min; digest scratchpad/dedupe/review/r4-digest.md): STANDING CL-1 BLOCKER (a mid-list overlay insertion breaks the slot-prefix test for later reads → the CURRENT composition's handle is 'new' → squared; order-dependent), DL-2 BLOCKER (a handle read before a union that adds universes carries stale members: mu_up__jes_up 6.3 vs 12.6), DL-1 BLOCKER (non-nominal projection: the ancestor join re-indexes the entry to its member and the union overwrites the projected nominal), DL-3 MED (cost bullet's node-count equality false under interning), FSB-4 MED (memo-hit/adopted read paths have no frozen witness), FSB-5 MED (D5's tag-map merge redundant per entry), CL-3 MED (r4's 'ids permute, as today' false — v3's members serialize identically across orders, main's do not). Refuted: CL-2, CL-4, DL-4, FSB-1, FSB-2, FSB-3. PLAN r5: records over FACTOR slots (overlays never change the nominal) with per-slot GENERATIONS (bumped by a universe-adding union); stale handle REFUSED, union over an overlay's prefix REFUSED (loud, naming the order that works); `_same_node` through vary links + nominal projections only; D5 merge dropped; cost bullet = bound; adopted-read leg; IR-equality sentence. Prototype v4 dispatched.
- m57 prototype v4 2d5c7b3: records keyed on the handle's MEMBER MAP (D21 — nominal-keyed records could not detect staleness since a join keeps the nominal node; caught by v4tx.py), stale refusal raised in `_vary_weight` (D22), overlay prefix read off the list (D23), generations count unions not node stamps (D24), union guard on the join / staleness guard on the registration, each naming the other order (D25). rs5b/rs5c one-SF both spellings/orders; D8b read-before → refused, read-after 12.6; universe(ctx,'hf_up') join → new factor, projected nominal preserved (= main 4.95); tag merge removed; 15/15 member IR equal across orders (main 3/15). DL-3 second half FALSE as stated: a left-assoc walk mints FEWER nodes (interning shares prefixes: 63 vs 309 at F=64) — composition work (record_op calls) separates them (1.24 vs 16.57 at F=64) → §4 cost leg counts recorded operations, node bound kept as a ceiling. Full runner exit 0; nosilent byte-identical; 146 frozen vary tests green. Plan r6 (D21–D25 + cost leg). Delta round r4→r6 launching.
- m57 delta review r4→r6 wf_310c97df-b8d (31 agents, 43 min; digest scratchpad/dedupe/review/r6-digest.md): 7 standing, ALL plan-text/§4-oracle defects, no implementer line changes (v4 already behaves as decided): RC-2/DL3-3/FS3-1 (one defect — the weight-label projection clause pinned "unchanged in value and node", false on every tree; the true oracle is projected universe × new central = main's answer, v3 erased it), DL3-1/FS3-2 (§3's match sentence still said "whose nominal is that node" — the rejected key; staleness leg needs both spellings since a nominal-keyed record accepts after an intervening read), RC-1/FS3-5 (the adopted-member recording return had no witness — the masked-child clause takes the memo hit; a nominal-projection read leg added), nit FS3-7 (zero-mint assertion needs a live control). Refuted: RC-3, DL3-2, DL3-4, DL3-5, FS3-3, FS3-4, FS3-6. Plan r7 = exit of the delta loop (zero design findings) → WHOLE-ARTIFACT PASS next.
- m57 WHOLE-ARTIFACT PASS wf_22091e98-091 (23 agents, 42 min; digest scratchpad/dedupe/review/whole-digest.md): 8 standing / 2 refuted / 4 NITs. BLOCKERs: WC-1 a handle read BEFORE a row-space change (mask or `graphed.nominal`) and registered at the child is a NEW factor → squared nominal at both class ends (`_adopt_ambient` reset `_reads`; the adopted head's node IS the handle's nominal at a nominal projection); WA-1 = WC-2 a length-1-prefix handle routes to the factor arm → the union widens the nominal member in name only (same nodes) → generation bump + both refusals fire on admitted programs (both orders of two relative-delta families refused at prefix 1, pass at prefix 2; the r2 "with one factor the two coincide" sentence ignored the join's side effects); WF4-1 a central whose nominal member is a container (public `Varied(...)`) JOINS at depth 2 on v4 where main raises GraphedTypeError (`_two_level` peels two; the guard fired only at depth 3; r2's FS-4 refutation premise "not constructible" falsified). MEDs: WC-3/WF4-2 the conflict leg's mint-count control reads 0 vs 0 (an accepted same-shape registration mints nothing until the ambient read); WF4-3 the transactional leg (repeated tag) fires before any join machinery → cannot fail; WA-2 §1's "0 hits" false — m52 point fixtures + m53b off-grid fixtures + one m53 extra test register a weight() handle (over one factor); their ambients change value, their label/point assertions do not.
- m57 plan r8 (scratchpad/dedupe/review/plan-r8.md, delta-r7-r8.diff): the recorded read decides FIRST (a one-factor handle is an overlay; a stale record refuses even where a factor would match); `_adopt_ambient` stores the head's tuple (parent live slots + gens at adoption) and ancestors' records stay reachable — a cross-change record equal to the head's tuple is an overlay after the head (nominal = the head's node), any other cross-change record is refused naming `weight(child)`; a container-valued nominal member names nothing (form check refuses as main); §4: prefix-length ends (1 and 2) in both orders + fresh-handle generation leg, row-space-change legs (mask + nominal projection, failing direction = squared nominal) + the prefix/grown-parent refusals, the conflict leg's instrument = Session node count with the control an accepted registration + ambient read, the transactional leg on the union-conflict refusal with a pre-read handle witness, nested control = refusal; §1 names the fixture set by instrument (scratchpad/dedupe/handle_centrals.py); NITs folded (§3 payload details dropped, count → property). v5 dispatched to Agent dedupe-prototype. Loop policy: one delta round (r7→r8 + v5); if it returns zero design findings the unchanged parts keep the whole pass's coverage (inputs unchanged) → test author.
- m57 prototype v5 b064312 (report v5.1–v5.6, D26–D33): reads walked before factors (stale beats a factor match, D26); one-level nested guard (D27); `_head` tuple on the context + `_lineage_reads` walk (D28–D31; one head answers the whole crossed walk — a record equal to an intermediate adoption is refused, D29/D33); crossed-record refusal `("elsewhere")` names `weight(child)` (D30). All whole-pass probes at target (older_handle pre=1 → ('ambient',1) + oracle, gens {} ; P3d nominal [0.5,1,2]; depth-2 nested → GraphedTypeError as main; b14d GREEN; P6k control 0 vs 1 across the read). Runner exit 0 byte-identical to v4's log; ruff/format/mypy clean; nosilent 4/4. NEW MEASURED COST (v5.3): an overlay replaces at every label carrying its coordinate INCLUDING a later family's PLACED universe — the tour diagonal `scale_upup` (points= {scale: upup, muR: 2, muF: 2}, declared member A·1.5) loses its declared member (v5 0.78 = 1.3·0.6; v4 at a 2-factor prefix already 1.56 — a standing overlay property the 1-factor join had hidden); tour cells 34/36 values move, 14/38 identical. D32: the two extra joints at a nominal projection (`mu_up__pu_up`) are m53 fan-out of the members' parent labels, same on main → left.
- m57 plan r9 (review/plan-r9.md; delta-r8-r9.diff = the placement decision; delta-r7-r9.diff = this round's delta): an overlay never replaces at a universe another family PLACED (the point registry names the placing family = the label's own); §2.5 placements keep their declared member; §4 leg (both prefix lengths; fails on a build that replaces at every label carrying the coordinate); §4 nominal-projection leg states the m53 fan-out of the members' parent labels. v6 dispatched to dedupe-prototype; delta-review script workflows/scripts/m57-plan-delta-r9.js ready (V6SHA placeholder).
- m57 prototype v6 efbb7c5 (report v6.1–v6.5, D34–D38): `_compose_ordered`'s span test gains `not _placed_elsewhere(overlay, label)` — the point registry does NOT carry the placing family's own axis (`_route`'s additive branch registers `scale_upup` under {muF: 2, muR: 2}), so the test is an absence: a placed universe is a label whose registered point carries no coordinate that renders it; the overlay's own universes (incl. joints it minted) are told apart by membership → plan r10 restates §3 (review/plan-r10.md, delta-r9-r10.diff). Tour cells 14/34/36/38 value-identical to the v4 record (only the probe's own `factors=` diagnostic moves); P14 on the declared diagonal at both prefix lengths (the 2-factor row was wrong on v4 too); P5_tourchain 0 mismatches; runner exit 0 byte-identical to v5's log; 253-probe sweep v5→v6: 4 behavioural differences (the placement legs). PRE-EXISTING (main == v6, review/whole/compose/P18_foldfreeze.py): a weight() read before a final append leaves a foldable memo, the append folds onto it, and a label minted after the memo resolves against it to nominal instead of its point's coordinates → being SIZED by dedupe-prototype (read-only copy); rides with v7 if small and confined to the fold memo.
- m57 DELTA REVIEW r7→r10 on v6 LAUNCHED wf_9d93002d-f6e (script workflows/scripts/m57-plan-delta-r10.js: composition/frozen/code lenses Opus xhigh + 2 Opus-high refuters per finding; P18 declared pre-existing in the context). Watcher incident: dedupe-prototype stacked five identical until-loops on one sweep sentinel; four killed, instruction issued (one waiter per event, foreground `timeout` for short waits); the sweep completed and the last waiter exited on its own.
- m57 P18 SIZED (dedupe-prototype, copies under scratchpad/dedupe/p18/): mechanism = `Varied._key_for` restricts a label's registered point to the container's OWN axes and matches an own label exactly; a composed (memoised) container merges every factor's axes, so a two-coordinate placement survives the restriction, matches nothing → `nominal`; `_two_level` memoises the miss as settled; `_vary_weight` stamps the folded result. Fix = a guard on the fold choice in `_vary_weight` (`_resolvable(base, factors)`: refuse the fold when a label this call minted restricts to a non-empty point on the base's axes and the base answers nominal) — 13 logic lines (+isort/docstring), confined to context.py's fold path; P18 both legs [14.04, 28.08, 42.12, 56.16]; frozen m52/m53/m53b/m56 + extras 271 passed; the fold still fires elsewhere (64 families: ops 2335, 120 ms vs 128 ms); no false positive constructible (single-coordinate restrictions always match an own label). DECISION: rides with v7 (small, confined) — plan r11 (§2.5 repair, §3 fold clause, §2.6 exception, §4 leg; review/plan-r11.md, delta-r10-r11.diff). v7 commit HELD until the r10 delta round completes (the reviewers probe the worktree tip).
- m57 DELTA REVIEW r7→r10 on v6 wf_9d93002d-f6e (31 agents, 51 min; digest scratchpad/dedupe/review/r10-digest.md): 6 standing / 7 refuted / 3 NITs. BLOCKER R8-C1: after a family JOINS an ancestor's factor at a masked child (the head expanded into the re-indexed parent list, `_adopted` cleared), `_lineage_reads` stops at the non-adopted child → every crossed record unreachable → a crossed handle (the tour idiom) falls through to NEW → nominal [1.0, 4.0] for pu·SF=[1.0, 2.0]; the other order is right; the strict-prefix crossed refusal is lost the same way (silently accepted, pu squared). MEDs: R8-C2 the conflict leg's control is dead with one live factor (accepted+read mints nothing; needs ≥2 factors and a read before the count); S4-1 the "parent grew" crossed spelling is unreachable on one lineage (refused by the pre-existing divergence check on every build) → use the WIDENED spelling; S4-2 the placed-universe leg cannot fail on a build that exempts by the point alone (needs the overlay family placing its OWN universe at a purely foreign point, P15's shape); DR-3 the transactional leg passes an eager-stamp build if `weight()` is read before the relative-delta registration (re-records under the moved generation); LOW S4-5 the nested control must be the bare `Varied({"nominal": container})` spelling. Refuted: DR-1 (blast-radius set — the headline fixture IS in §1's set; NIT residue), DR-2 (`_child_of` enumeration — mutant of a note, not of the design), S4-3/S4-4, DR-4..7. HOLDS: read-decides-first breaks no join-decided program (R4), overlay==join at a placement (R8), stale beats factor at prefix 1 (R2), head vs stale (R3), the four crossed cases (R5), P1/P5/P9/P12–P17/b* all on the oracle; tour cells value-identical; nosilent 4/4; determinism; docs example byte-identical. SWAP TRAP again: a reviewer left the worktree detached at 2d5c7b3 (v4) with a dirty context.py (+114/−27) — diff saved to review/r8/swap-leftover-2d5c7b3.diff, worktree restored to dedupe/prototype efbb7c5 (branch ref was intact).
- m57 plan r12 (review/plan-r12.md; delta-r11-r12.diff; the next round's delta = delta-r10-r12.diff): crossed records are matched like any record — slot prefix + current generations — because the head stands for the ancestor's (slots, gens) and an expansion leaves them in the child's list re-indexed; the record walk crosses every row-space link whether adopted or since expanded; a strict-prefix crossed record EXPANDS the head (via `_extend`'s ancestor branch, slots kept) and anchors after its prefix, nominal = the recorded node re-indexed (the D30 "elsewhere" refusal class is gone); stale (widened) refused as at the parent; the divergent-branch read stays the pre-existing row-space refusal. §4: both orders of (ancestor join, crossed overlay); strict-prefix crossed anchoring; widened-spelling refusal; conflict instrument with ≥2 factors + read before the count; transactional order (registration right after the refusal, no read between); own-placement clause; bare nested spelling. NITs folded (entries→values; no literal `weight(child)` spelling).
- m57 prototype v7 e1fe0d8 (context.py +76/−51; report v7 section D39+): P18 fold guard (`_resolvable` at the fold choice; 3.6× on the one call that mints a multi-coordinate placed label, whole-program unmoved ~247 ms) + r12 crossed-record rule (record walk crosses every row-space link; strict prefix expands the head via `_extend`'s ancestor branch, `('prefix', slot)`; `elsewhere` refusal class removed). R1 both orders on the oracle; R1c both orders anchored (mu_up = 1.05·pu·SF on the child's rows); R5 (a) strict prefix ANCHORED, (b) divergent-branch refusal (pre-existing), (c)/(d) unchanged. Runner exit 0 (66 sections, byte-identical to v6's log); ruff/format/mypy clean; nosilent 4/4; 346-probe sweep v6→v7: 14 intended differences. P3b_chains' stored wants were stale (chain B's older handle mints m53 fan-out joints `mu_up__pu_up`/`mu_up__hf_up`, same labels on main, main's values doubled). The swap-leftover diff was v6's context.py over a v4 checkout (nothing of the implementer's). S4-2/S4-5/DR-3 re-spellings hold with failing directions on v7 mutants; R8-C2's r12 spelling does NOT discriminate (a read before the count warms the memo → compose-to-decide mints nothing; re-indexing-to-decide is invisible to a count on the flat fixture) → plan r13: the conflict leg counts on a COLD memo (≥2 factors, no read since the last registered; refused 0 across the call; control = accepted + read), and the byte-identity leg's program includes a masked-child registration whose central is not the parent's (a re-indexing/composing build mints there). r13 = review/plan-r13.md, delta-r12-r13.diff; this round's delta = delta-r10-r13.diff.
- m57 DELTA REVIEW r10→r13 on v7 LAUNCHED (script workflows/scripts/m57-plan-delta-r13.js; composition/frozen/code lenses Opus xhigh + 2 Opus-high refuters).
- m57 DELTA REVIEW r10→r13 on v7 wf_2de44751-390 (21 agents, 44 min; digest review/r13-digest.md): 5 standing / 4 refuted / 3 NITs. BLOCKER L13-1 = R13-C1: an expansion at a masked child re-indexes the ancestor's entries → their nominal node ids move → `_same_node` no longer matches an ancestor-built central → only the FIRST ancestor-naming registration dedupes (K_min.py: hf on sf at the parent, lf and mf on sf at the child → nominal SF² [4,16]; main SF³); pre-existing in v6 for join→join, and r12's strict-prefix expansion opened overlay→join (B3_strictprefix: overlay then join → [2.0, 8.0] for [1.0, 2.0]); the record arm survives (keyed on slots), the factor arm (node ids) does not; at a nominal projection the class does not arise (ids preserved). MEDs: L13-2/S13-2 the byte-identity leg cannot catch an interned re-index (the accepted path re-indexes the same central; the code lens's mut_reindex that mints extra IS caught) → drop the delegation/premise, keep the shape; S13-1 the conflict leg's control, run first, interns the product a composing build would mint → refused leg first or separate Sessions. HOLDS: R8-C1 both orders at mask and projection; two crossed handles; two-mask chains incl. after inner expansion; crossed handle at a universe projection now accepted; widened/stale discriminates (mut_nogen); P18 both legs incl. under an overlay; cold-memo conflict count (refused 0 / accepted+read 4 / mut_compose refused 3); nosilent + mut_reindex/mut_compose move counts; transactional order; bare nested; own placement.
- m57 plan r14 (review/plan-r14.md; delta-r13-r14.diff): each re-indexed entry keeps the nominal it had in every row space it came through (beside the slot; a join keeps the nominal node) and the factor arm compares the central against the identity in its own row space; §4: two families on one ancestor central at the child, two on different ancestor factors, join after a strict-prefix crossed overlay in both orders (fails on a build that re-indexes without keeping identity); conflict leg refused-first/own-Session; byte-identity leg keeps the masked-child shape, drops the re-index premise; NITs (fold clause deleted, count→property, `_extend` route dropped). v8 dispatched; script m57-plan-delta-r14.js ready (V8SHA).
- m57 prototype v8 41adf06 (context.py +58/−8; report v8.1–v8.6, D48 = identities via `_prior_nominals`, a sibling walk, not a 4th `_live_factors` return → plan r15 wording): per-slot identity nominals per row space crossed; K_min [2.0, 4.0] one entry; B3 all three mask legs; B6 child == parent; N1 two ancestor factors OK; N2 two-mask chain with top- and middle-built centrals OK (v7 4 entries [256,1296]; last-row-space-only identity build 3 entries [64,216] — discriminates); sweep v7→v8 405 probes: 13 intended (K_min, B3, B4, B5_doublejoin, B6, F_respell, N1, N2, X12–X16 — X12/X13: the widening-under-a-covering-overlay refusal now fires in the expanded-list spelling where v7 silently minted a new factor), 31 noise; identity map costs no nodes/time. S13-1 verified (composing build fails with the refused leg first or in its own Session; passes only control-first same-Session). Runner exit 0 (66 sections); mypy/ruff clean; nosilent 4/4; docs.patch + Sphinx -W clean. Waiter incident: the agent's single background waiter was killed at the harness's 10-min limit while the job completed → nudged; rule: re-arm from short self-expiring checks.
- m57 DELTA REVIEW r13→r15 on v8 LAUNCHED (script workflows/scripts/m57-plan-delta-r15.js).
- m57 DELTA REVIEW r13→r15 on v8 wf_7903dd1d-e14 (19 agents, 31 min; digest review/r15-digest.md): 6 standing (3 lenses × 2 findings: the same two) / 3 refuted / 4 NITs. BLOCKER L15-1=F15-1=R15-C1: at `graphed.universe(ctx, L)` (non-nominal) a central built ABOVE the projection matches the ancestor's entry (`_live_factors` walks through the projection's adoption; both nodes live above it so `_same_node` sees no link; the identity chain bypasses the guard too) → join → the union nominal replaces the projected member → the universe projected into is silently erased (P_N_univ_erasure: hf named inside hf_up → nominal [0.5,1,2] where the projected ambient was [0.55,1.1,2.2]; main keeps it, squared); the cross row (pu named inside hf_up) joins correctly and keeps the projection; standing since v6; §4's control used the re-derived central (names nothing) so the suite was green. MED L15-2=F15-2=R15-C2: the "one identity per row space" half has no leg — a last-row-space-only build passes all three r14 legs; N2 (two-mask chain) discriminates. NITs: the failing-direction figure attached to three legs holds for two; docs clause "the row space it came from" (singular). HOLDS: K_min/N1/N2/B3 all on the oracle, P_G/P_H chains, conflict ordering (composing mutant fails refused-first/own-Session, passes control-first), `_prior_nominals` walks the same chain, docs example byte-identical.
- m57 plan r16 (review/plan-r16.md; delta-r15-r16.diff): a projection to another universe L keeps the entries' identity; a central built above names its factor there and the expansion projects each entry to its member at L, EXCEPT the factor that OWNS L (a family of its container is a coordinate of L's point) — refused, naming the parent-side registration and the re-derived central; §4 projection legs (cross row keeps pu·SF(up_hf); owning row refused; each ± a prior masked-child expansion); the two-mask identity leg; parenthetical → property. Fourth blocker in the row-space-change seam (records reset → records unreachable → factor identity lost → projection identity crossing); v9 dispatched; script m57-plan-delta-r16.js ready (V9SHA).
- m57 prototype v9 f70ed00 (report v9.1–v9.5): the factor arm refuses a match whose factor OWNS a label projected into (ownership by the label's POINT — a mutant deciding by the label's own family passes plain labels and fails the joint leg: a third family placed at {hf: up, pu: up} → both owners refused, `tr` joins and keeps the projected ambient [135, 270, 540]); P_N hf row → GraphedError naming both spellings, pu row keeps [0.55,1.1,2.2]; REF1 four rows split as specified; P_K (re-derived central) unchanged (new factor); ± a masked-child expansion before the projection, a second projection, a mask on either side: same; a projection to a SHIFT label is not an ownership case (tag maps carry weight families only; U3's jes_up row byte-identical). The record arm needs no rule (a handle read above the projection → ('ambient', 1), projected nominal unmoved, mu_up = 1.05× it, m53 fans out the parent's labels as for any pre-change handle; O4_record_arm byte-identical v8→v9). Sweep 432 probes v8→v9: 13 = the refusal firing, 31 noise. Runner exit 0 (66 sections); mypy/ruff clean; nosilent 4/4; docs.patch: R15-N2 fixed ("every row space it came through") + the projection clause; Sphinx -W exit 0. Process: the sweep runs in ~3 min as foreground chunks, run-tests.sh ~7 min foreground — no waiters needed.
- OWNER HOLD in force: no r15→r16 review round, no test-author dispatch, until the owner's signal. Script m57-plan-delta-r16.js is filled (tip f70ed00) and ready to launch.
- 2026-09-10 owner signal → hold lifted (session 3415dc0a). r15→r16 delta review on v9 launched (wf_a683e09c-2e7; Opus xhigh lenses, Opus high refuters). Worktree clean at f70ed00 before launch.
- m57 DELTA REVIEW r15→r16 on v9 wf_a683e09c-2e7 (21 agents, 35 min, 2.2 M tokens; digest review/r16-digest.md, durable copy session-12dd48ff-handoff/m57/review/): 7 standing = 2 design defects × 3 lenses + 1 MED; 2 refuted (F16-3 re-derived spelling, F16-4 family names); 1 NIT. BLOCKER A (L16-1c=L16-2f=F16-1): v9's ownership refusal read `factor._tags` off the RE-INDEXED entry, which the project re-index (`member_of`) empties → after ONE expansion at/below the projection the owner joins and erases the projected universe (r17/A7_minimal.py: v9 nominal [0.5,1,2] where the projected ambient was [1.5,3,6]; main keeps the projection, squared) — the L15-1 class still live, coordinate repair. BLOCKER B (L16-2c=L16-1f=F16-2): "the expansion projects each entry to its member at L" false when the entry's dependence on L is not a member — L a shift label (one-level re-index drops the node-carried jes dependence), L an overlay's own label (rescale dropped; r17/A3_overlay_own_label.py nominal [0.5,1,2] vs [0.525,1.05,2.1]). MED L16-3f: owner leg one-coordinate labels only. Same seam as every blocker since the whole pass (expansion of the head across a row-space change), not a new class → loop continues, but the fold is the NARROW one.
- m57 plan r17 (review/plan-r17.md, delta-r16-r17.diff; +18 lines): the head is NEVER expanded across a projection to a non-nominal universe — the factor arm refuses a match whose entry's row space lies above one (identity kept), the record arm refuses a strict-prefix record read above one; the whole-head record still anchors; the error names the spellings that work (register/read at the parent before projecting; read at the projection); the r16 ownership rule + entry-projection re-index deleted; §4 projection legs re-spelled as refusals for L a weight/shift/overlay label with the node-identical-ambient witness; §2.6 lists the new refusals. Evidence the cross row is exotic: no frozen/extra test, tour cell or docs example registers a weight family inside `graphed.universe(ctx, L)` (grep; the tour projects the WEIGHT); m55 already refuses a parent-built container at a projected child. Prepared, NOT launched (owner: "prepare for the next review but do not launch"): workflow-scripts/m57-plan-delta-r17.js (V10SHA placeholder; backtick trap caught by a wrapped node --check), m57/v10-brief.md (fresh Opus-high implementer), m57/test-author-prompt.md.
- OWNER 2026-09-10: asked whether a provenance RIDER (per-entry history) solves r16's A and gives mechanisms for B; answered yes for A (the reviewers' class repair: the family set is the last per-object state read after a re-index — v9 already keeps slots/kind/gens/identity per slot; the project re-index is a one-level `member_of` in accessors `_follow`), partially for B (B(a) two-level re-index; B(b) needs a decision: inside an overlay's own universe an operation inside its covered prefix has no correct answer under replacement overlays without minting a ratio node → refuse). Owner: "Fold this in as a consideration — this makes the design more robust (and also more easily investigatable by humans)."
- m57 plan r18 (review/plan-r18.md; delta-r17-r18.diff and, for the review, delta-r16-r18.diff — r17 never reviewed, superseded): §3 the RIDER beside each slot (kind, families with coordinates, identity per row space, overlay prefix, history of mask/project links), never read off the entry object; `graphed.context.ambient_entries(ctx)` → (slot, rider, entry) for people and the frozen suite; §2.3 the r16 cross-row join at a non-nominal projection restored and decided from riders (factor → two-level member at L; owner of L refused, joint/placed labels have several owners; overlay with a coordinate on L → its member, FIXED so `_compose` replaces at every value; an operation inside its covered prefix refused; a join after the overlay multiplies; overlay without a coordinate left out); §4 projection legs re-spelled (cross row; owner before/after a cross-row join and after a masked-child expansion; placed two-family label; shift label two-level; overlay's own universe ×5; rider leg); §2.6 names joined-once and refused programs; W1/W2 rows. 448 lines (+42 over r16: the rider + reader are scope). Prepared, NOT launched: m57/v10-brief.md (r18), workflow-scripts/m57-plan-delta-r18.js (V10SHA).
- OWNER 2026-09-10: "In the next prototype round we should also implement an inspector for the rider/provenance so that users can see how their sources of uncertainty are getting rendered into the final list of variations." → plan r19 (review/plan-r19.md; delta-r18-r19.diff; the review delta is delta-r16-r19.diff): §2.7 `graphed.explain(ctx)` → `Explanation` record + one-line-per-item text: (a) families in registration order (name, Kind, tags, registering context's links from the root, entry form: shift + collections / new factor / join naming the factor's other families / overlay over the families read over / placements with points); (b) the operation list from the riders; (c) every ambient and shifted-collection label with its origin (one-at-a-time, fan-out joint by its point's families, placement, overlay universe) and per weight family the families it composes with (complement of the families in its minted labels); derived from registered points + riders, never re-deciding; mints nothing beyond a weight read; text has no node ids (byte-identical across Sessions). §3 names what it reads (`_weight_tags`, shift registry, placements, riders, link chain, registered points); §4 explain legs (tour chain entry forms + origins; masked/universe child links; m56 capstone fan-out vs compose; node count; two-Session text identity); W1/W2. Named `explain` (not `provenance`: the glossary's provenance = node→source location; `inspect` = M9). Scope sent to the running v10 implementer (SendMessage) + appended to m57/v10-brief.md; review script re-targeted as workflow-scripts/m57-plan-delta-r19.js (r18 script removed).
- m57 prototype v10 c385085 (Agent dedupe-v10, Opus high; report v10.1–v10.8, D68–D73): riders per slot (context.py +591/−84: riders 319/82, inspector 273/3; `__init__` +3; `_check_forms` spy arity), projection re-index by rider, `ambient_entries`, `graphed.explain` (registration record = new context state so explain never re-decides; one line per registration; operations numbered by position, slots only in the record — slot ids are process-global; `_path_from_root` collapses vary runs; covered families sorted; non-context refused with GraphedError). VERDICT matrix 34 legs (review/r18impl/matrix.txt): v10 34 OK; each behavioural leg WRONG on ≥1 of v9/v8/main except the re-derived-central control. Sweep 432 probes v9→v10: 1 behavioural (P_J: ('factor',3) → ('owned','pu_up'), r18's outcome), 2 ImportError on the deleted `_prior_nominals` (shim: v9 verdicts), 24 noise. Gates: run-tests EXIT=0 (66 sections), ruff/format 0, mypy strict 0 (branch config; tests under main's #27 config = 100 pre-existing errors byte-identical to v9 → rebase step), sphinx -W 0 with docs.patch. Lead re-ran r17/A7_minimal.py and A3_overlay_own_label.py on v10: all REFUSED. V10SHA filled in workflow-scripts/m57-plan-delta-r19.js. Review NOT launched (owner hold).
- 2026-09-10 owner viewed explain's rendering ("This will already help when probing the code on the next review and prototyping rounds. Good!") and had two wording defects folded: plan r20 (review/plan-r20.md; delta-r19-r20.diff; review delta delta-r16-r20.diff) — §2.7 "composes with" = WEIGHT families only, shifts reported as fanned-out-over / independent-of; an operation with no row-space link says so in words (v10 printed "via -"); §4 capstone leg gains "pure-weight family independent of jes, never composing with it". Fixup v10.1 requested from dedupe-v10 (new commit on c385085; record gains shift fields; docs block re-matched). Review script rebuilt as workflow-scripts/m57-plan-delta-r20.js (a failed anchor + rm lost the r19 script; rebuilt from r17 with the full chain, node-checked; V10SHA placeholder awaits the fixup sha). The r19/r18 scripts no longer exist; r17's stays as the chain's base.
- m57 prototype v10.1 3824681 (fixup on c385085, context.py +54/−16; D74: a shift family's own line reports no relations, its mirror sits on every weight line): `Family` gains `fans_out_over`/`independent_of`, `composes_with` weight-only; "registered here" for an entry that crossed no link; P8 +5 legs (39 total, all OK on v10; unreachable on c385085 — AttributeError on the new field); runner EXIT=0 (66 sections), ruff/format/mypy clean, sphinx -W 0, docs block byte-identical (no shift in the example), sweep name set identical to c385085's. Lead verified tip + runner + P7 rendering. V10SHA = 3824681 filled in workflow-scripts/m57-plan-delta-r20.js. Review NOT launched (owner hold).
- 2026-09-11 owner: launch r16→r20 but replace the two-refuters-per-finding stage with ONE adjudicator; asked for a rebuttal if wrong. Measured from the r16 transcripts: lenses 3 agents / 34.8 M input (incl. cache) / 0.16 M output / 298 turns; refuters 20 agents / 42.7 M / 0.27 M / 618 turns — and 9 of 18 assignments were the same two defects raised by three lenses. No rebuttal: the adjudicator keeps integrity if every verdict carries its own measurement and it defaults to STANDING when the counter-example cannot be built; it gains cross-lens clustering (no split votes). Script rewritten: lenses in parallel (barrier, the adjudicator needs all findings) → one Opus-xhigh adjudicator (clusters, measures, writes review/r20/adjudication.md as it goes; null result → every raised finding stands unadjudicated). Trap fixed pre-launch: `${S}` inside a single-quoted JS string does not interpolate → concatenated. LAUNCHED wf_88340746-0fc (task w3fecm004) on v10.1 3824681, worktree clean.
- OWNER RULE 2026-09-11: fine-grained fan-out only on sonnet/haiku; on Opus/Fable consolidate into few long-running agents (one adjudicator, not N refuters). Recorded in memory subagent-model-effort-tuning.md; applied to the m57 impl-review step (handoff §4.6) — the lens fan-out (3 distinct long-running perspectives) stays.
- 2026-09-11 r16→r20 review wf_88340746-0fc PAUSED by the owner (TaskStop) with all THREE lenses complete (3 result rows in the journal) and the adjudicator started; worktree clean at 3824681. Resume same-session with resumeFromRunId (lenses replay from cache; only the adjudicator re-runs); a new session launches fresh.
- m57 DELTA REVIEW r16→r20 on v10.1 wf_88340746-0fc DONE (3 lenses + 1 adjudicator; digest review/r20-digest.md, adjudication log review/r20/adjudication.md): 15 lens findings → 12 clusters, ALL STANDING, 1 NIT. BLOCKERS: R20-A a FIXED overlay is dropped at a further non-nominal projection (`_reindex_entry` tests `_spans` on the re-indexed member, never `rider.fixed`; join moves the nominal by 1/1.05 — same class: a rule reading a re-indexed object); R20-B `_extension`'s first candidate is the re-indexed entry's own nominal, so a central node-identical to a factor's projected member is a new factor while adopted and joins the OWNER after an unrelated expansion (order-dependent; §4's control pins hf's nominal expression, stable in both orders); R20-C NEW CLASS: `explain` (and a plain `weight()` read) RECORDS the ambient, and at a projected context nothing records the adopted member until a read, so a later family on `universe(weight(ctx), L)` flips new-factor → overlay with the read — §2.7 neutrality, §2.1 read-neutrality and the Out-of-scope clause all false there; absent on main. MEDs: R20-D explain's relations/placements computed from labels HERE vs families of the LINEAGE (inverts at a projection); R20-E "independent of jes" printed for a weight family registered BEFORE the shift (the shift-after-weight diagnostic's own case); R20-F "composes with" printed for two families sharing one factor (should say shares); R20-G `ambient_entries` at a projection lists the un-re-indexed list (dropped overlay shown; unstable across a value-neutral join); R20-H §4 shift-label leg spelled non-discriminating (join the factor WITHOUT the shift coordinate); R20-I rider leg needs an expansion AT a projection; R20-J two-Session text-identity leg cannot fail (same program → same ids; pin "no integer node id in the text"). LOW: R20-K slot in the printed line vs byte identity (print position); R20-L "both orders agreeing" holds at masked child only, not at a nominal projection (label sets differ). Stopping rule: R20-C is a NEW class → brought to the owner before folding.
- 2026-09-11 owner: the rider solves ordering + dependency tracking → agreed the physics choice (a central naming the universe projected into is REFUSED as the owner) → plan r21 (review/plan-r21.md; delta-r20-r21.diff; 548 lines): §3 the PROVENANCE PRINCIPLE (every nameable node recorded when it comes to exist — nominal per row space, member at each projected label, compositions handed out by reads AND adoptions; decisions read provenance, never live entries; a grep gate); rider gains the projected member + fixed persists (R20-A/B); `_adopt_ambient` records the adopted member as the child's read (R20-C; §2.1 read-neutrality extends to children; Out-of-scope clause qualified); ambient_entries = composed operations (R20-G); explain: lineage points (R20-D), "shares the factor" (R20-F), shift-after-weight wording (R20-E), position (R20-K), no integer node id (R20-J); §4 legs re-spelled (R20-H/I + new legs); "both orders agreeing" scoped (R20-L). v11 DISPATCHED (Agent dedupe-v11, Opus high, brief m57/v11-brief.md). Review script prepared: workflow-scripts/m57-plan-delta-r21.js (V11SHA; delta r20→r21; adjudicator shape) — NOT launched.
- OWNER 2026-09-11: code organisation — a `graphed.systematics` package holding the vary machinery, `graphed.vary` the entrypoint, narrow submodule imports for exact work. Measured layout at v10.1: context.py 2134 lines (EventContext + the whole ambient machinery + explain), vary.py 797, varied.py 423, accessors.py 276; cycles vary↔context and accessors↔context via deferred imports; frozen suites import `graphed.context` (14 files) and `graphed.varied` (7), no test imports the `graphed.vary` module. Plan r22 = r21 + W6 (module map: kinds/tags/points/varied/accessors/by_label/registration/ambient/explain; EventContext stays in graphed/context.py calling systematics.ambient; one-line shims for every old path; ONE refactor commit of pure moves after the v11 round is clean and before the test author; gates: git diff -M, runner, lint/types/Sphinx, byte-identity sweep). The r21 review script reads delta-r20-r21 (design); W6 is organisation, reviewed by the impl review's code lens.
- m57 prototype v11 2a0c297 (Agent dedupe-v11, Opus high; context.py +291/−81; report v11.1–v11.4, D75–D82): riders record the member at each projected label (written by the projection; a mask adds an identity, a projection does not — D76/D78); `_extension` reads rider nodes only (projected member → owner refused / non-owner joined once, D75); `_adopt_ambient` records the adopted member as the child's read; `_entry_state` (rider.fixed else families vs the label's point) drives `_reindex_entry` and `ambient_entries`; `_spans` deleted; explain: lineage points via `_minted_by` (D82), "shares" on both lines (D80), shift-order phrase replaces independent_of (D81); `_covers` placement exemption read off the point (D77, retires D61). Matrix 32 legs: v11 32 OK; v10.1 15 OK/8 WRONG/9 absent; v9 6/5/7 unreached/14; main 4/7/7/14; 5 all-tree controls. Sweep v10.1→v11: 11 r21 outcomes (5 R20-C, 6 R20-B), 24 noise. Gates: runner EXIT=0 (66), ruff/format clean (116 files), mypy strict clean (tests = v10.1 baseline), sphinx -W 0, determinism 4/4, §2.6 nosilent byte-identical to v10.1 and main. Provenance grep: 21 residual `_two_level` sites, all composing/tag-map/identity-recording (report v11.3). Reviewer probe note: r20lens/B1 leg 2 fails identically on v10.1 — its oracle contradicts §2.3's owner rule (pre-existing). Lead re-ran S23 (refuses both orders), R2_two_projections (ratio 1.0). V11SHA = 2a0c297 in workflow-scripts/m57-plan-delta-r21.js; NOT launched.
- 2026-09-12 owner: "why would you wait for the refactor?" → no reason; W6 refactor DISPATCHED now (Agent systematics-refactor, Opus high, brief m57/refactor-brief.md) on top of v11 2a0c297, so the r21 round reads the final layout and the test author is unblocked right after a clean round. Compatibility gate: 370 review probes import private names from graphed.context (_extension 229, _two_level 185, _live_factors 70, …) — the shim keeps them importable; sweep vs v11 must be noise-only. After it lands: plan §3 module names → r23; review CTX names the new modules; then the round on the owner's go.

## 2026-09-12 — m57: W6 refactor landed (9e5a7e9), plan r23, r21 review launched then paused; lens-merge analysis
- W6: vary machinery moved to graphed.systematics (pure move, shims at old paths, sweep noise-only, runner/ruff/mypy/sphinx clean). Plan r23 = §3 module paths only.
- r20→r21 review (script m57-plan-delta-r21.js, run wf_8311a691-c76) launched on tip 9e5a7e9, stopped for a laptop pause mid-lenses; resume note in SESSION-RESUME-STATE.md.
- Owner asked whether the 3 lenses can be one agent. Measured over 9 rounds/27 lens transcripts (m57/review/lens-merge-analysis.md): serial merge costs 2.1–2.3× input and would cross the ~400k compaction point every round; 14/21 blocker defects were found by exactly one lens (per-lens recall ≈0.49); the refuter→adjudicator merge saved because refuters were short and prefix-dominated, lenses are not. Interim recommendation: keep the three lenses, one adjudicator. r21 numbers to be appended.
- r20→r21 review DONE (wf_9e60b010-b8e, continuation of the killed run; 4 agents, 43 min): 19 raised → 16 clusters, 0 refuted: BLOCKER R21-A (expansion rebuilds the child's list in `_live_factors` walk order, moving an anchored overlay past a child-registered factor → overlay universe ÷ that factor; plan text already demands order preservation), R21-B (recorded projected member compared with `_same_node`, which refuses across non-nominal project links, so a parent-built node-identical member becomes a NEW factor → squaring; the nominal-prior arm already crosses the link); 11 MED (E: the §3 provenance grep is false on v11 and cannot fail — §4 legs are the real guard; I: adoption clause squares through a mask; K: "composes with" definition ≠ code; L/M: §4 leg gaps; C/D/F/G/H/J explain and leg issues); 3 LOW. Digest review/r21-digest.md, verdicts review/r21/adjudication.md (copied to m57/review/). Next: fold → r24, v12 brief.
- Lens-merge question answered from data (m57/review/lens-merge-analysis.md, 10 rounds/30 lenses): keep the 3 lenses separate; merge = 2.1–2.3× input, 2.5–3× wall, 415–627k end context past the ~400k compaction point; 16/23 blockers single-lens, per-lens recall ≈0.48; adjudicator refutes 0/28 vs refuters 25% — flag, not a verdict.
- r24 round (v12 1eac07c): 8 clusters, no blocker → exit round; r25 folds the 4 MED + 4 LOW (stacked-projection ownership over every universe the context is inside with the innermost match; nominal arm first when a node is both a nominal and a recorded member; record lives in the entry's own row space; label-set clause re-attached; no quoted renderings). Test author (plan r25) and v13 (repairs + rebase onto 9374bd8 + docs commit) dispatched in parallel.
- v13 (repairs 9468be0 + docs 9f05500, rebased on 9374bd8) green. Frozen suite authored in isolation (68 tests, 2ab6ad9) → cherry-picked 46497ba = freeze-m57; 32 fail on the prototype (record shape, values, refusals) → v14 implementer iterating.
- Re-freeze 667679d (owner-affirmed, 9 legs) → suite 68/68, runner EXIT=0. Tour updated (fork 9d4963f, Level 20 explain/riders; not pushed). Impl review: REJECT with 3 blockers (one code defect in the widening check after a projection re-index; two frozen legs that cannot fail in their direction, closed by extra legs) + 4 LOW; v15 closing them.
- m57 IMPL REVIEW CLOSED + PR: v15 closure at 2f623bb (widening-level class fix; 8 extra legs in tests/extra/awkward/m57/test_m57_review_closures.py — 7 kill one-hunk mutants the whole frozen tree survives, 1 is C-K's admitted member; docs import explain). Independent closure check (Opus xhigh, review/impl/closure.md) APPROVE: every cluster re-measured on the tip, C-B shown a class repair, 213 adj + 117 matrix legs zero regressions, gates green. Frozen m57 on the pre-m57 tree measured 60 F / 8 pass (all 8 README controls; scratchpad/dedupe/frozen-m57-on-main.log). Pushed dedupe/prototype + freeze tags; **graphed PR #28** opened 2026-09-12 ~10:00; enqueue watcher armed (pid 37538). Post-merge steps in SESSION-RESUME-STATE.md.
- m57 DONE+MERGED 2026-09-12: graphed PR #28 → main 079d92f (owner enqueued; merge-group CI green). graphed-histogram #14 (pin
  079d92f, arg-type ignore dropped; 214 passed locally) → cec8911. W5 committed (superproject afad29f): systematics plan §2.1(b)
  nominal rule + §2.5/§2.6/§9.1 corrections, root prompt R24.5, dedupe plan r26 — fact-checked by two probe-driven lenses
