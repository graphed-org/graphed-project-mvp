...
Status: **draft for review (r20).** Anchoring doc for the work, structured like the root prompt:
...
- **§1.1** Verb `graphed.vary`; container `graphed.Varied`; concept "variation" everywhere (docs,
  ...
  **e-encoding** (owner-selected 2026-07-30, three-option encoding decision): the canonical
  ...
  computation also rejects); r18: the input grammar states no magnitude
...
- **§2.2 (`Varied` is a mapping of universes; extraction is functional.)** `Varied` holds
  `{label: Array}` with `"nominal"` always present. **It additionally retains, PER `name`, the tags
  ...
  **module functions** (the r6 namespace-collision principle, §2.6a): `graphed.labels(x)`
  ...
  labels, (b) the labels of any `Varied` collections on the context, and (c) the labels of
  **`graphed.selection(ctx)` in §9.1's sense — the selection reached by skipping over any number of
  ...
  and its per-link-kind ANSWER is bound there independently of when the verb is exported) — by
...
- **§2.3 (Broadcast propagation — five bound dispatch points.)**
  ...
  **`graphed.reindex_to` (the §6.1d lineage seam, NEW in m48, r20) BROADCASTS** — a `Varied` value
...
- **§6.1 (MVP shape: sibling fills, named results.)** `Histogram.fill` accepting `Varied` axis
  ...
  **This is the shape of the UNPACKED user-facing result, NOT of the plan's combined value, and the
  ...
  (r19)**: an output NO variation reaches keeps today's BARE `output_name` key. r18's unqualified
  ...
  **WHERE the `S`/`W` split is witnessable is bound too, r20, because the sibling formula cannot see
  ...
  result still equals its sibling-fill decomposition.** Without it an implementation classing a
  ...
  **positionally** over the distinct-output list (`boost.py:100-117`, `for j in range(i, i + k)`,
  ...
  node id **replicates** into every slot that needs it instead of shifting positions. **That list
  is obtained through §7.2's bound `aggregate_plan` seam, r19** — measured, `aggregate_plan`
  compiles INTERNALLY (`python/graphed/aggregate.py:95`) and takes `reduce`/`empty` as pre-built
  closures (`:68-108`), while `plan()` builds `layout` before calling it
  (`graphed-histogram src/graphed_histogram/boost.py:282,286-291`), so the indices are not
  computable at the site r18 assumed.
  **The AXIS-MODE slot is bound here too — and it is scoped to m50, with §6.2** (r15; scoping added
  ...
  confidently-wrong class). m48's anchor names that answer.
  ...
  **(B) `graphed.reindex_to(value, ctx)`-shaped** — returns `value` re-expressed in `ctx`'s row
  ...
  dispositions: `reindex_to` **broadcasts** (a `Varied` value gives a `Varied` with the same labels,
  exactly like `graphed.broadcast_like`); `unify_contexts` takes context handles rather than
...
- **§6.4 (Variation-aware write-out — skim augmentation; NEW scope, collaborator-directed
  ...
  **the supplied mask's own §2.3e context handle MUST EQUAL the record's context handle —
  `graphed.context_of(select_mask) is graphed.context_of(record)`** — a single comparison of two
  values both operands already carry, with no traversal at all. It is the same property (2a) was
  reaching for: a mask whose reads were performed through the record's context lives in the
  record's row space by §2.6c, which is exactly what the level-0 OR requires. Verified against all
  four m51 controls: canonical skim `to_parquet(events.Jet, select=graphed.selection(sel))` with
  ...
  carries no handle (§2.3e Drop rule) so it can never equal a contexted record's handle → (ii)'s
  refusal. The r16 admission of any number of `vary` IDENTITY links becomes automatic — a `vary`
  link does not change which context a read was performed through — and the r17 per-label
  **node-id-equality** rule for "the supplied mask IS `graphed.selection(c)`" is WITHDRAWN with the
  ...
  from an ancestor or a sibling context is refused at the `to_parquet` call, naming both contexts.
...
- **§7.2** The frontend owns `(output, label) → **node id**` — NOT `→ position` — and derives
  ...
  **THE SEAM THAT MAKES THIS DERIVATION POSSIBLE IS BOUND HERE, AND IT IS AN m48 IMPLEMENTATION
  TARGET (r19)**, because three binding requirements need the compiled artifact at a site that does
  not have it: §6.1c's index-based `layout` is "derived frontend-side from the compiled output list"
  yet is a constructor argument of `_GroupReduce`/`_GroupZero`, which are passed INTO
  `aggregate_plan`; this section's own m48 optimizer-merge refusal must compare distinct compiled
  ...
  **`aggregate_plan` gains ONE pinned seam (exact spelling pinned at m48 freeze) that (α) lets the
  ...
  before the worker closure is finalized, and (β)
  ...
  *`CompiledGraph`, not ids*: §6.1c's layout and this section's own merge refusal need only
  `node id -> position`, which `outputs()` would give, but the seam's THIRD named consumer does not
  ...
  frontend-side `compile_ir` before `aggregate_plan`'s own would double it. The seam is a function
  ...
  **Each half is anchored in the repo whose source it is, r20** (the seam is new source in
  ...
  anchor over an UNVARIED `aggregate_plan` build** (§10/m48), and **(β) carries its anchor at m49
  alongside §8.2(i)**, whose `variation_labels` is its only consumer — until then it is a
  pass-through with a default, not unreachable source. **The same node-id
...
- **m48 — `vary` frontend + weight path** (repos: `graphed` + `graphed-histogram`).
  ...
  compiled output list is not available where §6.1c's layout and §7.2's own merge refusal need it —
  measured, `aggregate_plan` compiles internally at `python/graphed/aggregate.py:95` and takes
  pre-built closures; §7.1/§7.3/§7.4 stay m49), §6.3, **§9.1 partially —
  ...
  **AND except the VARIED half of its §7.2 merge-guard clause, r20** (§7.2 r18 binds that guard's
  ...
  `src/graphed_histogram/boost.py:256-292`, verified the sole owner of `(output_name, Histogram)`
  ...
  **Three anchors straddle the split and are assigned explicitly** (r12 — r11 pinned the
  ...
  - **§7.2's r19 `aggregate_plan` SEAM (α), in `graphed`'s `tests/frozen/frontend/m48`** (r20 — the
    ...
    other `graphed` m48 anchor calls `aggregate_plan`, and `graphed`'s only existing coverage of it
    is m5's): over an UNVARIED multi-output program in the m5 call shape
    (`tests/frozen/frontend/m5/test_aggregate_plan.py:77`), the new hook fires EXACTLY ONCE and
    receives the `CompiledGraph` — its `outputs()` readable BEFORE the worker closure exists — and
    ...
    unchanged). Seam half (β) is anchored at m49 with §8.2(i).
  ...
  - **§6.1a result shapes** — **in SIBLING mode** (§6.2 axis mode is m50 and has its own shape,
    ...
    `-> dict[str, bh.Histogram]` at `:101-118` — is UNCHANGED, which §6.1a's own varied slot keying
  ...
  - **§7.2 schema absence**: the `ExecResult`/`Plan`/monitor payload schema **KEY SETS** on a varied
    ...
    key sets, NOT over plan bytes or the `process` spec — m49's §8.2(i) closure field changes those
    by design (§7.2, §7.3).
  ...
  - §2.6/§6.1d event-context anchors (r6 functional form): ambient fill on a per-object quantity —
    ...
    **context-handle ORIGINATION** (§2.3e r14 — the merge-from-inputs rule alone gets this wrong,
    ...
    `ValueError: spans must have compatible lengths`); **`unweighted=True` in §6.1d's r19
    bound form** — it suppresses the AMBIENT weight AND any explicit `weight=[…]` (a contexted fill
    with registrations yields counts equal to an unweighted eager reference), and
    ...
    **alongside the INTEGER-MAGNITUDE rejection as a distinct case** (r17 — §1.1 binds two
    ...
    (33 plain digits) is REJECTED with the magnitude message. The ACCEPTED half is the discriminating
...
- **m50 — scale + integration** (repos: `graphed-histogram` + `graphed` preserve/docs).
  ...
  - Variation-axis fill equals sibling-fill results bin-for-bin on the corpus **weight** labels,
    ...
    sample column reused across universes. **Plus the per-fill CARRIER witness** (§6.2 r15):
...
- **m51 — variation-aware write-out (skim augmentation)** (repos: `graphed` +
  ...
  - **Entry checks (§6.4a, r12 — TWO predicates, each anchored with the positive control IT
    ...
    **(2a) lineage, record-time** — a record whose context is not the one the supplied
    ...
    wording froze a REFUSAL for it, hard-blocking a legal skim spelling read-only (§6.4a(2a) r20); **(2b) row-count equality between the record and that mask — EXECUTION-time, per
