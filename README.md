...
| **M5** | ✅ DONE | graphed-awkward-mvp, graphed-numpy-mvp, graphed | Necessary-buffer (column) projection |
| **M6** | ✅ DONE | graphed-debug-mvp | graphed-debug: opt-level lowering, source-mapped tracebacks, viz |
| **M7** | ✅ DONE | graphed-executors, graphed-core-mvp | Execution-layer contract + graphed-exec-local |
| **M8** | ✅ DONE | graphed-checkpoint-mvp, graphed-core-mvp, graphed | graphed-checkpoint: plan serialization + checkpoint/resume |
| **M9** | ✅ DONE | graphed-preserve-mvp, graphed-core-mvp, graphed, graphed-awkward-mvp | graphed-preserve: analysis preservation bundle |
| **M10** | ✅ DONE | graphed, graphed-core-mvp, graphed-awkward-mvp, graphed-executors, graphed-orchestrator | Remediation of the mvp-shortcomings findings: IR-driven execution (compile_ir/evaluate_ir, one dispatch per REDUCED node), genuine incremental sessions (Session(incremental=True)), buffer-level projection, full A.5 CI matrices + build-only wheels + cargo llvm-cov, opt-in maximal fusion + pooled combines, Partition.blind, and an integrity scan that catches bare-pass targets |
| **M11** | ✅ DONE | graphed, graphed-numpy-mvp, graphed-awkward-mvp | dask.array parity P0: full ufunc tier + backend-idiom factorization (array_type; NumpyArray proxy; awkward stays functions-only) |
| **M12** | ✅ DONE | graphed, graphed-numpy-mvp | dask.array parity P1: axis-aware reductions/scans (boundary vs fusible structural rule), creation routines, deterministic random, tree-reducible monoids |
| **M13** | ✅ DONE | graphed, graphed-numpy-mvp | dask.array parity P2: indexing (slices/ints common; tuple subscripts numpy-side) + manipulation with axis-0 geometry rules + concatenate/unique/bincount/histogram* |
| **M14** | ✅ DONE | graphed, graphed-numpy-mvp | dask.array parity P3.8: graphed.apply multi-input blockwise externals + signature-aware apply_gufunc (P3.9 + P4 deferred to Phase 2 by user decision) |
| **M15** | ✅ DONE | graphed, graphed-awkward-mvp, graphed-numpy-mvp | Partitioned parquet I/O: backend-agnostic base in graphed (blind partitions, deferred write plans) + awkward specialization (schema-only forms, buffer-projection-wired writes) + numpy rectilinear specialization (R16.7 amended by user decision: parquet alone enters the numpy MVP) |
...
| **M17** | ✅ DONE | graphed, graphed-awkward-mvp | dask-awkward parity P1: ~20 structure functions through the single apply dispatch + the common record-subset getitem (a[[fields]]) |
...
| **M20** | ✅ DONE | graphed | Partitioned-write base: a format-agnostic graphed.write that client integrations specialize (parquet in the backends, ROOT in the reader fork) instead of borrowing parquet tools — a deferred write_plan with worker-reported paths, blind partition indexing, and exact step reconstruction |
...
| **M22** | ✅ DONE | graphed, graphed-core-mvp | Output-scoped compiles: graph outputs are a property of the compile REQUEST, not session/store state — fixes the compile_ir output-accumulation footgun; sequential compiles from one session/store are history-independent and byte-identical to fresh-session compiles |
| **M23** | ✅ DONE | graphed, graphed-histogram | Deferred histogram filling (graphed-histogram, the dask-histogram analogue): a histogram fill is a preservable External node; graphed gains record_external(descriptor=, form=) so backends stay domain-free |
...
| **M29** | ✅ DONE | graphed-histogram | Multiple multiplicative weights are a first-class fill signature: fill(weight=[...]) records each weight as a graph input (params carry n_weights) and evaluation multiplies them elementwise; a single weight records exactly as before |
...
| **M31** | ✅ DONE | graphed-executors | Ship the process callable to each worker ONCE, not per task: the driver pickles the compiled-IR-bearing process once and broadcasts it (cached by content hash), submitting only (token, partition) per task — results and determinism unchanged |
...
| **M34** | ✅ DONE | graphed-executors | Bounded shared-process cache + dedup: the M31 ship-once broadcast cache is FIFO-bounded in lockstep with the driver's token set (full worker coverage asserted before caching), reusing graphed-core's one bounded LocalResources |
| **M35** | ✅ DONE | graphed | Session.walk evaluates iteratively (explicit stack): a deeply-chained selection/systematics graph materializes far beyond Python's recursion limit instead of raising RecursionError (materialize and projection share walk) |
...
| **M37** | ✅ DONE | graphed-core-mvp, graphed-executors, graphed-debug-mvp | Live execution dashboard (FINOS Perspective + websockets + a network transport): passive Monitor seam (graphed-core, unchanged), executor emit (graphed-exec-local), and graphed-debug's DashboardServer (perspective Server + Tornado) + NetworkMonitor (websocket, loopback or remote) + Dashboard + pyinstrument profile rows. Determinism gate green attached-or-not. Root prompt R20. (SSE/uPlot prototype on m37-sse-uplot.) |
| **M38** | ✅ DONE | graphed-core-mvp, graphed-executors | Inter-worker comms: a WorkerTransport seam (graphed-core) plus IPC/HTTP backends with peer tree-reduction and work-stealing (graphed-exec-local); peer is the default (comms='ipc') and reduces bit-for-bit identical to the hub path. Root prompt R21 |
| **M39** | ✅ DONE | graphed, graphed-executors | Distributed joins I — the `Exchange` (shuffle) IR boundary + `ShuffleBackend` protocol with a keyed all-to-all over the p2p transport (merge-to-large-blocks, never M×R). Root prompt R22. |
| **M40** | ✅ DONE | graphed, graphed-executors | Distributed relational joins — a `Join` IR boundary + `JoinBackend`, `graphed.join(on,how)` with SQL-duplicating semantics and a deterministic broadcast-vs-shuffle cost model; bounded-memory join + skew salting. Root prompt R22. |
| **M41** | ✅ DONE | graphed, graphed-executors | Cluster-runtime hardening — fetch RAM budget + reader-plane spill, incast coalescing, per-node disk budget, tested cross-process over routable addresses behind a `ClusterExecutor` launch seam (real launcher deferred). Root prompt R22. |
| **M42** | ✅ DONE | graphed-executors | Dask executor backend — a `SubmitBackend` protocol driving reference execution over `dask.distributed`. |
| **M43** | ✅ DONE | graphed-executors | Dask backend completion — full graphed-on-dask execution + resume parity with the reference executor. |
| **M44** | ✅ DONE | graphed-executors | `DaskWorkerTransport` — graphed's `WorkerTransport` seam atop dask worker-to-worker comms. |
| **M45** | ✅ DONE | graphed-executors | Unified shuffle facade — one entry point selecting the shuffle transport via `shuffle_method`. |
| **M46** | ✅ DONE | graphed-executors | Parsl backend I — a parsl executor adapter with a self-built HTTP rendezvous transport (parsl has no p2p). |
| **M47** | ✅ DONE | graphed-executors | Parsl backend completion — full parsl execution + mutation-verified acceptance. |
| **M48** | ✅ DONE | graphed, graphed-histogram | Systematics-vary I — the `graphed.vary`/`Varied` frontend + weight-path lowering (record-time expansion, no Rust `Vary` NodeKey); sibling weight fills. Root prompt R23. |
| **M49** | ✅ DONE | graphed, graphed-histogram, graphed-executors | Systematics-vary II — shift-path lowering + impact introspection + cross-process executor end-to-end vs the corpus references. Root prompt R23. |
| **M50** | ✅ DONE | graphed-histogram, graphed | Systematics-vary III — scale + integration: the sink variation axis (`StrCategory`) fill lowering + varied preservation bundle. Root prompt R23. |
| **M51** | ✅ DONE | graphed | Systematics-vary IV — variation-aware skim write-out: `graphed.awkward.to_parquet(select=)` superset base + XOR-delta/packbits universe columns under a parquet-KV manifest (+ uproot-fork derived-column ROOT half). Root prompt R23. |
...
| [graphed](https://github.com/graphed-org/graphed) | consolidated package: frontend + core (Rust IR/optimizer) + awkward + numpy + debug + checkpoint + preserve | `559ed8d` | ✅ submodule |
| [graphed-corpus-mvp](https://github.com/graphed-org/graphed-corpus-mvp) | M0.5 requirements + fixtures | `c5997b3` | ✅ submodule |
| [graphed-executors](https://github.com/graphed-org/graphed-executors) | graphed-executors: reference + dask + parsl executors, shuffle/join data plane | `bc91f11` | ✅ submodule |
| [graphed-histogram](https://github.com/graphed-org/graphed-histogram) | histogram filling (M23/M29) + systematics-vary sink variation axis (M48-M51) | `46264f4` | ✅ submodule |
| [graphed-orchestrator](https://github.com/graphed-org/graphed-orchestrator) | Part B deterministic orchestrator | `3323e53` | ✅ submodule |
...
<sub>Generated from <code>.graphed/state.json</code> (updated 2026-09-19T18:18:46Z).</sub>
