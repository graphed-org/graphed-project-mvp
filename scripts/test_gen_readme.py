...
def test_repo_states() -> None:
    out = gen_readme.render(STATE, PINS)
    assert (
        "| [graphed-core-mvp](https://github.com/graphed-org/graphed-core-mvp) | M1/M4 core | `101cb8d` | ✅ submodule |"
        in out
    )
...
def test_clean_repos_strips_annotations() -> None:
    assert gen_readme._clean_repos(
        ["graphed-orchestrator (DONE)", "graphed-corpus"]
    ) == [
...
def test_meta_repo_sorted_first() -> None:
    out = gen_readme.render(STATE, PINS)
    rows = [
        ln for ln in out.splitlines() if ln.startswith("| [") and "github.com" in ln
    ]
...
def test_live_state_renders_and_matches_committed_readme() -> None:
    ...
    assert gen_readme.README_PATH.read_text() == out, (
        "run scripts/gen_readme.py to refresh README.md"
    )
