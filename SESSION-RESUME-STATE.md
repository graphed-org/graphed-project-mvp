...
## Update 2026-09-19 — integration arc (coffea / fastjet / conda): graphed side SHIPPED, waiting on upstreams
...
- **Forks, all LOCAL, review loops closed** — open each upstream PR only when its gate is met: uproot `~/vibe-coding/uproot5-graphed-form-mapping` @ `graphed-form-mapping` 6bdc948 ← scikit-hep/uproot5#1720 merged (U); fastjet `~/vibe-coding/fastjet-graphed-mvp` @ `graphed-arm` 7751717 ← scikit-hep/fastjet#400 merged (F1); coffea `~/vibe-coding/coffea-graphed-mvp` @ `graphed-mvp` 9c6cace8 ← a uproot release carrying #1720 + U, then raise coffea's uproot floor (A1).
...
- Awaiting others: conda-forge reviewers on #34887; uproot5#1720 review; fastjet#400 merge. Owner confirmations outstanding: fastjet arm REFUSES flat single-event input; coffea arm has no `.graphed` descriptor slot.
