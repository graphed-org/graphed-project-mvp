...
## Update 2026-09-19 — integration arc (coffea / fastjet / conda): graphed side SHIPPED; fastjet#401 + coffea#1637 + uproot5#1720 in upstream review

Journal: `coffea-fastjet-conda-worklog.md` (§6 Execution, `tail`); plan `coffea-fastjet-conda-plan.md`; `m58`–`m60-decomposition.md`.
- **DONE:** m58–m60 merged (graphed main 559ed8d: #31 c8f58e2, #32 653e404, #33 dfe4936, #34 5c283e5 incl. owner-affirmed m25 re-freeze `freeze-m25-fixup`, #35 559ed8d); **0.0.3 series on PyPI** (graphed 559ed8d, histogram 46264f4, executors bc91f11); MIT LICENSE in all six repos; conda-forge/staged-recipes#34887 at 0.0.3 (head bb2e883, fork `lgray/staged-recipes@graphed-stack`).
- **Forks (review loops closed):** uproot `~/vibe-coding/uproot5-graphed-form-mapping` @ `graphed-form-mapping` 6bdc948 = FOLDED into scikit-hep/uproot5#1720 (owner 2026-09-19; ff push to `graphed-org:graphed-mvp`, description + comment updated); fastjet `~/vibe-coding/fastjet-graphed-mvp` @ `graphed-arm` f7d5d55 = **scikit-hep/fastjet#401 OPEN** (rebased on merged #400; docs example fixed); coffea `~/vibe-coding/coffea-graphed-mvp` @ `graphed-mvp` 9c89d7b1 = **scikit-hep/coffea#1637 OPEN** (owner: open early; CI installs uproot from `refs/pull/1720/head`; before merge: delete that step + raise the uproot floor at the uproot release, drop the `exclude-newer-package` opt-out after 2026-09-26).
- Dev envs: `~/vibe-coding/integ/venv` and `<scratchpad>/fj/venv-src` read graphed from the detached worktree `~/vibe-coding/integ/graphed-pin` (re-pinned to main 559ed8d = 0.0.3).
- Awaiting others: conda-forge reviewers on #34887 (CI green at 0.0.3); uproot5#1720 review (CI green with the fold); fastjet#401 + coffea#1637 review + CI. Owner confirmations outstanding: fastjet arm REFUSES flat single-event input; coffea arm has no `.graphed` descriptor slot.
- Traps met this arc: `prek -a` in the uproot fork rewrites `pyproject.toml` (toml-sort 0.25) — use `--files`; `pytest --cov` dies in the integ venv — `coverage run --branch -m pytest`; implementer agents may commit in scratch clones — check the canonical clone; stacked-PR / merge-async facts in memory `merge-queue-enqueue-precondition`.
